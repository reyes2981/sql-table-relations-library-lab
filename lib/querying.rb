...
def select_value_and_count_of_most_prolific_species
  "SELECT species, COUNT(species) FROM characters GROUP BY species ORDER BY COUNT(species) DESC LIMIT 1;" #group and then order by
end

def select_name_and_series_subgenres_of_authors
  "SELECT authors.name, subgenres.name FROM subgenres LEFT OUTER JOIN series ON subgenres.id = series.subgenre_id LEFT OUTER JOIN authors ON authors.id = series.author_id;"
end
